...
// eslint-disable-next-line no-console
console.log('Bigquery credentials:', credential);
// eslint-disable-next-line no-console
console.log(
  'Bigquery envs:',
  BIGQUERY_DATASET_ID,
  BIGQUERY_PAGEVIEW_TABLE_ID,
  BIGQUERY_EVENT_TABLE_ID,
...
export async function insertBigQueryData(data, table) {
  try {
    // eslint-disable-next-line no-console
    console.log('Bigquey data:', data, table);
    // Insert the data into the table
    await bigquery.dataset(BIGQUERY_DATASET_ID).table(table).insert(data.data);
